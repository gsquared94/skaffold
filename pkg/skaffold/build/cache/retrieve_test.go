...
type mockArtifactResolver map[string]string

func (m mockArtifactResolver) GetImageTag(imageName string) (string, error) { return m[imageName], nil }
...
func (b *mockBuilder) BuildAndTest(ctx context.Context, out io.Writer, tags tag.ImageTags, artifacts []*latest.Artifact, existing []build.Artifact) ([]build.Artifact, error) {
	var built []build.Artifact
	r := make(mockArtifactResolver)
	...
	for _, artifact := range artifacts {
		...
		_, err := b.dockerDaemon.Build(ctx, out, artifact.Workspace, artifact.DockerArtifact, artifact.Dependencies, tag, config.RunModes.Dev, r)
		if err != nil {
			return nil, err
		}
...
func TestCacheBuildLocal(t *testing.T) {
	testutil.Run(t, "", func(t *testutil.T) {
		...
		bRes, err := artifactCache.Build(context.Background(), ioutil.Discard, tags, artifacts, nil, builder.BuildAndTest)
