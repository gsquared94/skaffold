...
func TestWithLogFile(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			digest, err := builder(context.Background(), &out, &latest.Artifact{ImageName: "img"}, BuilderOptions{Tag: "img:123"})
...
func fakeBuilder(_ context.Context, out io.Writer, a *latest.Artifact, opts BuilderOptions) (string, error) {
	fmt.Fprintln(out, "building", a.ImageName, "with tag", opts.Tag)
	return "digest", nil
}

func fakeFailingBuilder(_ context.Context, out io.Writer, a *latest.Artifact, opts BuilderOptions) (string, error) {
	fmt.Fprintln(out, "failed to build", a.ImageName, "with tag", opts.Tag)
