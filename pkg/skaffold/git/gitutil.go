...
// SyncRepo syncs the target git repository with skaffold's local cache and returns the path to its configuration file
...
func syncRepo(g latest.GitInfo, opts config.SkaffoldOptions) (string, error) {
	...
	return filepath.Join(repoCacheDir, g.Path), nil
}
