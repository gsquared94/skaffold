...
func TestGetDeployments(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: []*resource.Rollout{
				resource.NewRollout("dep1", "test", 10*time.Second),
				resource.NewRollout("dep2", "test", 20*time.Second),
			},
		},
		{
			...
			expected: []*resource.Rollout{
				resource.NewRollout("dep1", "test", 300*time.Second),
			},
		},
		{
			...
			expected: []*resource.Rollout{
				resource.NewRollout("dep1", "test", 100*time.Second),
				resource.NewRollout("dep2", "test", 200*time.Second),
			},
		},
		{
			...
			expected: []*resource.Rollout{
				resource.NewRollout("dep1", "test", 200*time.Second),
			},
		},
		{
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			t.CheckErrorAndDeepEqual(test.shouldErr, err, &test.expected, &actual,
				cmp.AllowUnexported(resource.Rollout{}, resource.Status{}),
...
func TestGetDeployStatus(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			deployments: []*resource.Rollout{
				resource.NewRollout("foo", "test", time.Second).
		...
		},
		{
			...
			deployments: []*resource.Rollout{
				withStatus(
					resource.NewRollout("deployment", "test", 1),
...
func TestPrintSummaryStatus(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			testEvent.InitializeState([]latest.Pipeline{{}})
			r := withStatus(resource.NewRollout(test.deployment, test.namespace, 0), test.ae)
...
func withStatus(d *resource.Rollout, ae proto.ActionableErr) *resource.Rollout {
	d.UpdateStatus(ae)
	return d
}
...
func TestPollDeployment(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			dep:         resource.NewRollout("dep", "test", time.Second),
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			pollRolloutStatus(context.Background(), &statusConfig{}, dep)
