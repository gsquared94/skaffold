...
KEY_ID=$(gcloud iam service-accounts keys list --iam-account=metrics-writer@k8s-skaffold.iam.gserviceaccount.com --project=k8s-skaffold --managed-by=user --filter="validAfterTime>-P1W" --format="value(name)" --limit=1)
if [ -z "$KEY_ID" ]; then
